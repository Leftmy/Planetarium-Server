...
**`roadmaps_desc_trgm_idx` лишається (перевірено 4 серпня 2026, BE-08).** Було припущення, що GIN по `description` зайвий, бо колонка має `DEFAULT ''`. `EXPLAIN ANALYZE` на 20 000 рядків, де третина описів порожні, це не підтвердив: на селективному запиті планувальник бере `Bitmap Index Scan` по цьому індексу, а на реальному каталожному запиті `title ILIKE ... OR description ILIKE ...` — `BitmapOr` по обох GIN-індексах одразу. Порожні рядки не дають триграм і не роздувають індекс.

Що справді варто знати: індекс по `description` — 7 МБ проти 4 МБ самої таблиці, тобто більший за неї, і кожен `UPDATE` опису його оновлює. Це усвідомлена плата за пошук по каталогу. Друге обмеження — шаблони коротші за три символи (`ILIKE '%go%'`) триграми використати не можуть і завжди дають seq scan; якщо каталог має підтримувати короткі запити, це вирішується на рівні usecase (мінімальна довжина запиту), а не індексом.
